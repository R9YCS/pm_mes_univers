...
interface PrintersMonitorProps {
  ...
  onAssignOrder: (printerId: number, orderId: number) => void;
  onCompleteJob: (printerId: number, jobData: any) => void;
}
...
const AssignJobDialog: React.FC<{
  ...
  open: boolean;
  onClose: () => void;
  onAssign: (printerId: number, orderId: number) => void;
  ...
  const availableOrders = orders.filter(order => {
    ...
    const isAlreadyAssigned = order.printer_id && order.status_id === 3; // статус "В печати"
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAssign(printer.id, parseInt(selectedOrderId));
    onClose();
    setSelectedOrderId('');
  };
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
                        <span>Вес: {order.estimated_weight || '?'}г</span>
                        <span>Время: {order.estimated_time || '?'}мин</span>
                        {order.deadline && (
...
const CompleteJobDialog: React.FC<{
  ...
  open: boolean;
  onClose: () => void;
  onComplete: (printerId: number, jobData: any) => void;
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="mb-4 p-3 bg-gray-50 rounded">
          ...
          <div className="space-y-1 text-sm">
            <p>Заказ: {currentOrder.order_number}</p>
            <p>Название: {currentOrder.name}</p>
            <p>Плановое время: {currentOrder.estimated_time || '?'} мин</p>
            <p>Плановый вес: {currentOrder.estimated_weight || '?'} г</p>
            {currentMaterial && (
              <p>Материал: {currentMaterial.name} ({currentMaterial.color})</p>
            )}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="success">Печать успешна</Label>
            </div>
          </div>
...
export const PrintersMonitor: React.FC<PrintersMonitorProps> = ({ 
  ...
  const handleDeletePrinter = (printerId: number) => {
    if (window.confirm('Вы уверены, что хотите удалить этот принтер?')) {
      ...
      onDeletePrinter(printerId);
      toast.success('Принтер удален');
    }
  };

  const handleAssignOrder = (printerId: number, orderId: number) => {
    onAssignOrder(printerId, orderId);
    toast.success('Задание назначено на принтер');
    setAssigningPrinter(null);
  };

  const handleCompleteJob = (printerId: number, jobData: any) => {
    onCompleteJob(printerId, jobData);
    toast.success('Задание завершено');
    setCompletingPrinter(null);
  };
        ...
        {filteredPrinters.map(printer => {
          ...
          return (
            <PrinterCard
              ...
              onAssignOrder={setAssigningPrinter}
              onCompleteJob={setCompletingPrinter}
            />
          );
        })}
